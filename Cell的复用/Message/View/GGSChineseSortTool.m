...
@implementation GGSChineseSortTool
...
+(NSMutableArray*)IndexArray:(NSArray*)stringArr{
    ...
    {
        NSString *pinyin = [((GGSChineseSortTool*)object).pinYin substringToIndex:1];
        //不同
...
+(NSMutableArray*)LetterSortArray:(NSArray*)stringArr{
    ...
    for (NSString* object in tempArray) {
        ...
        NSString *pinyin = [((GGSChineseSortTool*)object).pinYin substringToIndex:1];
        NSString *string = ((GGSChineseSortTool*)object).string;
        //不同
        if(![tempString isEqualToString:pinyin])
        {
            //分组
...
+(NSMutableArray*)ReturnSortChineseArrar:(NSArray*)stringArr{
    ...
    {
        GGSChineseSortTool *chineseString = [[GGSChineseSortTool alloc]init];
        ...
        chineseString.string = [GGSChineseSortTool RemoveSpecialCharacter:chineseString.string];
...
/** 数组元素的交换 */
+ (NSMutableArray *)changeIndexWithArray:(NSMutableArray *)dataArray
{
    id data = dataArray[0];
...
+(NSMutableArray*)IndexWithArray:(NSArray*)objectArray Key:(NSString *)key{
    ...
    // 处理返回数据
    A_Result = [GGSChineseSortTool changeIndexWithArray:A_Result];
    return A_Result;
}
...
+(NSMutableArray*)sortObjectArray:(NSArray*)objectArray Key:(NSString *)key{
    ...
    NSString *tempString;
    //拼音分组
    for (GGSChineseSortTool* object in tempArray) {
    ...
    LetterResult = [GGSChineseSortTool changeIndexWithArray:LetterResult];
    return LetterResult;
}
...
+(NSMutableArray *)ReturnSortObjectArrar:(NSArray *)stringArr key:(NSString *)key
{
    ...
    for(int i=0;i<[stringArr count];i++){
        ...
        //这里我自己写了一个递归过滤指定字符串
//        chineseString.string = [GGSChineseSortTool RemoveSpecialCharacter:chineseString.string];                                         // 这里使用的方法就是将昵称前面的特殊字符串过滤掉
...
+ (NSString *)getFirstLetter1:(NSString *)chinese{
    ...
    //把拼音按字分开  汉语转化为拼音的时候  每一个汉字之间都会以空格隔开
